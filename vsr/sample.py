...
from models.pipeline_stable_diffusion_upscale_video_3d import (
...
def main(args):
    ...
    pipeline = StableDiffusionUpscalePipeline.from_pretrained(
        args.pretrained_path + "/stable-diffusion-x4-upscaler",
    ...
    for i, video_name in enumerate(video_list):
        ...
        imageio.mimwrite(
            save_path, upscaled_video, fps=fps, quality=9
        )  # Highest quality is 10, lowest is 0

        print(
            f'Save upscaled video "{video_name}" in {save_path}, time (sec): {run_time} \n'
        )
