...
def get_input(args):
    ...
    if input_path is not None:
        ...
        video_frames = torch.zeros(
            16, 3, args.latent_h, args.latent_w, dtype=torch.uint8
        )
        args.mask_type = "all"
        video_frames = transform_video(video_frames)
        n = 0
    ...
    for seed in args.seed_list:
        ...
        for ckpt in [ckpt_path]:
            ...
            args.input_path_list = [
                os.path.join(args.input_folder, video) for video in video_list
            ]
            for input_path in args.input_path_list:
                ...
                if args.copy_no_mask:
                    ...
                    mask = mask_generation_before(
                        args.mask_type, video_input.shape, video_input.dtype, device
                    )  # b,f,c,h,w
