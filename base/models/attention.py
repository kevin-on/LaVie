...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            hidden_states = rearrange(
                hidden_states, "(b f) d c -> (b d) f c", f=video_length + use_image_num
            ).contiguous()
            hidden_states_video = hidden_states[:, :video_length, :]
            hidden_states_image = hidden_states[:, video_length:, :]
            norm_hidden_states_video = (
                self.norm_temp(hidden_states_video, timestep)
                if self.use_ada_layer_norm
                else self.norm_temp(hidden_states_video)
            )
            hidden_states_video = (
                self.attn_temp(norm_hidden_states_video) + hidden_states_video
            )
...
class TemporalAttention(CrossAttention):
        ...
        if self._use_memory_efficient_attention_xformers:
            ...
            if self._slice_size is None or query.shape[0] // self._slice_size == 1:
                hidden_states = self._attention(
                    query, key, value, attention_mask, time_rel_pos_bias
                )
            else:
                hidden_states = self._sliced_attention(
                    query, key, value, sequence_length, dim, attention_mask
    ...
    def _attention(
        ...
        query = self.scale * rearrange(
            query, "b f (h d) -> b h f d", h=self.heads
        )  # d: dim_head; n: heads
        key = rearrange(
            key, "b f (h d) -> b h f d", h=self.heads
        )  # d: dim_head; n: heads
        value = rearrange(
            value, "b f (h d) -> b h f d", h=self.heads
        )  # d: dim_head; n: heads
        ...
        attention_scores = (
            attention_scores - attention_scores.amax(dim=-1, keepdim=True).detach()
        )
        ...
        hidden_states = torch.einsum(
            "... h i j, ... h j d -> ... h i d", attention_probs, value
        )
